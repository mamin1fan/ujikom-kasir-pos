<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border p-5 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ $total_kelompok_kategori }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border">
                ...
                <div class="px-6 py-4 border-b flex justify-between items-center">
                    <div>
                        <h3 class="text-base font-semibold text-gray-800 dark:text-white">Daftar Kelompok Kategori</h3>
                        <p class="text-xs text-gray-400">Kelola kelompok kategori produk</p>
                    </div>

                    <flux:modal.trigger name="add-kelompok">
                        <button class="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm rounded-xl">
                            + Tambah Kelompok
                        </button>
                    </flux:modal.trigger>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Nama Kelompok</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Jumlah Kategori</th>
                        ...
                        <tbody class="divide-y">
                            @forelse ($kelompok_kategori as $item)
                                <tr class="table-row-hover">
                                    <td class="px-6 py-4 text-sm text-gray-400">{{ $kelompok_kategori->firstItem() + $loop->index }}</td>
                                    <td class="px-6 py-4 text-sm font-medium text-gray-800 dark:text-white">{{ $item->nama_kelompok }}</td>
                                    <td class="px-6 py-4 text-sm font-medium text-gray-800 dark:text-white">{{ $item->kategori_count }}</td>
                                    <td class="px-6 py-4 text-xs-truncate text-gray-500">{{ $item->creator->username ?? 'System' }}</td>
                                    <td class="px-6 py-4 text-xs text-gray-500">{{ $item->created_at->format('d M Y • H:i') }}</td>
                                    <td class="px-6 py-4">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center py-16 text-gray-400">Belum ada kelompok kategori</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                @if ($kelompok_kategori->hasPages())
                    <div class="px-6 py-4 border-t">{{ $kelompok_kategori->links() }}</div>
                @endif
    ...
    <flux:modal name="edit-kelompok" class="md:w-[420px]">
        <form id="editForm" method="POST">
            ...
            @method('PUT')
            <div class="space-y-4">
                <flux:heading size="lg">Edit Kelompok</flux:heading>
    ...
    @push('scripts')
        ...
        <script>
            ...
            function openEditModal(id, nama) {
                ...
                document.getElementById('editForm').action = ROUTES.update.replace(':id', id);
            ...
            function confirmDelete(id, nama) {
                Swal.fire({
                    ...
                    .then(async res => {
                        ...
                        if (!res.ok) {
                            Swal.fire({ icon: 'error', title: 'Tidak bisa dihapus', text: data.message });
                            return;
                        }
